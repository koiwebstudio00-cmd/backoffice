// Edge Function: reveal-credential
// Self-contained (no shared imports) so it can be deployed by pasting this single file
// into the Supabase dashboard editor. Decrypts one secret after validating an owner JWT and
// records the access in credential_access_log.
...
Deno.serve(async (req) => {
  ...
  if (!id) return jsonResponse({ error: 'Falta el identificador de la credencial.' }, 400)

  const service = serviceClient()
  const { data, error } = await service
    .from('credentials')
    .select('secret_ciphertext, secret_iv, notes_ciphertext, notes_iv')
    .eq('id', id)
    .single()

  if (error || !data) return jsonResponse({ error: 'La credencial no existe.' }, 404)

  try {
    const secret = await decryptSecret(data.secret_ciphertext, data.secret_iv)
    const notes = data.notes_ciphertext && data.notes_iv ? await decryptSecret(data.notes_ciphertext, data.notes_iv) : null
    await service.from('credential_access_log').insert({ credential_id: id, user_id: auth.userId })
    return jsonResponse({ secret, notes })
  } catch (decryptError) {
